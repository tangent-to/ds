...
describe('Plot Configuration Generators', () => {
  describe('ordiplot - PCA', () => {
    ...
    it('should generate PCA plot config', () => {
      ...
      const config = plot.ordiplot(pcaResult, { type: 'pca' });

      expect(config).toHaveProperty('type', 'ordiplot');
      expect(config).toHaveProperty('ordinationType', 'pca');
    ...
    it('should include loadings when requested', () => {
      const pcaResult = mva.pca.fit(data, { nComponents: 2 });
      const config = plot.ordiplot(pcaResult, { type: 'pca', showLoadings: true });
    ...
    it('should handle color grouping', () => {
      ...
      const config = plot.ordiplot(pcaResult, { type: 'pca', colorBy });
    ...
    it('should support custom dimensions', () => {
      const pcaResult = mva.pca.fit(data, { nComponents: 2 });
      const config = plot.ordiplot(pcaResult, { type: 'pca', width: 800, height: 600 });

      expect(config.width).toBe(800);
      expect(config.height).toBe(600);
    });

    it('should support loadingFactor parameter', () => {
      const pcaResult = mva.pca.fit(data, { nComponents: 2 });
      const config = plot.ordiplot(pcaResult, {
        type: 'pca',
        showLoadings: true,
        loadingFactor: 0,
      });
  ...
  describe('ordiplot - LDA', () => {
    ...
    it('should generate LDA plot config', () => {
      ...
      const config = plot.ordiplot(ldaResult, { type: 'lda' });

      expect(config).toHaveProperty('type', 'ordiplot');
      expect(config).toHaveProperty('ordinationType', 'lda');
      expect(config).toHaveProperty('data');
      expect(config.data.scores).toBeDefined();
    ...
    it('should handle 2D discriminants', () => {
      const ldaResult = mva.lda.fit(X, y);
      const config = plot.ordiplot(ldaResult, { type: 'lda', axis1: 1, axis2: 2 });
  ...
  describe('ordiplot - RDA', () => {
    ...
    it('should generate RDA triplot config', () => {
      ...
      expect(config).toHaveProperty('type', 'ordiplot');
      expect(config).toHaveProperty('ordinationType', 'rda');
      expect(config).toHaveProperty('data');
